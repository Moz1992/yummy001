...
async function setup() {
  ...
  const { error: tableError } = await supabase.rpc('pg_catalog.exec', { sql: createTableSQL }).catch(() => {
    // Try direct SQL if RPC doesn't work
    return { error: null };
  });

  // Try alternative: use REST API to check if table exists
  const { error } = await supabase.from('products').select('id').limit(1);

  if (error && error.message.includes('does not exist')) {
    console.log('Table does not exist. Creating via direct insert...');
    // Create table using raw SQL via Supabase Dashboard is needed
    // For now, let's try to create records and see
    const { data, createError } = await supabase.from('products').insert(products).select();
    if (createError) {
      console.log('Cannot create table automatically:', createError.message);
      console.log('Please create the products table in Supabase Dashboard with the following schema:');
